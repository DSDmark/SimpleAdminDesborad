...
const mailSender = async (to: string, url: string, txt: string) => {
  ...
  const mailTemplate = `<!DOCTYPE html><html><head> <meta charset="UTF-8"> <title>Email Template</title> <style> body { font-family: Arial, sans-serif; margin: 0; padding: 0; background-color: #f1f1f1; } .container { max-width: 600px; margin: 20px auto;  background: linear-gradient(to bottom, #000, #e6e6e6);border-radius: 5px;box-shadow: 0 2px 5px rgba(0,0,0,0.1);padding: 20px;} .header { background-color: #ffffff; padding: 10px 0; border-bottom: 1px solid #dddddd; text-align: center; } .content { padding: 20px 0; } .footer { background-color: #f1f1f1; padding: 10px 0; border-top: 1px solid #dddddd; text-align: center; } </style></head><body> <div class="container"> <div class="header"> <h1>Varification Mail 📫</h1> </div> <div class="content"> <p>Hello ${to},</p> <p>${txt}</p><a href=${url}><button style="color:white;background:red;padding:1rem;">Varify Yourself</button></a><p>Regards,</p> <p>AdminDesborad-CMSystem</p></div><div class="footer"> <p>&copy; 2023 AdminDesborad-CMSystem. All rights reserved.</p> </div> </div></body></html>`
  try {
